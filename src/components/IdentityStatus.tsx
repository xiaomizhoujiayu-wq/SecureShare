// ============================================================================
// IdentityStatus.tsx - Display user's cryptographic identity, UID and attributes
// ============================================================================
...
// Represents a single attribute with its display category, value and color theme
interface Attribute {
  category: string;
  value: string;
  color: string;
...
// Predefined color themes for attribute badges (cycled through)
const COLOR_THEMES = [
...
// ----------------------------------------------------------------------------
// Main IdentityStatus Component
// ----------------------------------------------------------------------------
export function IdentityStatus() {
  // UI states
  const [copied, setCopied] = useState(false); // Copy feedback flag
  const [loading, setLoading] = useState(true); // Loading indicator
  const [userUID, setUserUID] = useState("Loading..."); // User's unique identifier
  const [attributes, setAttributes] = useState<Attribute[]>([]); // Parsed attributes list

  // --------------------------------------------------------------------------
  // Fetch identity data from backend on mount
  // --------------------------------------------------------------------------
  useEffect(() => {
    const fetchIdentity = async () => {
      try {
        ...
        // Split comma-separated attributes, trim each
        const attrArray = rawAttrString.trim() ? rawAttrString.split(",") : [];

        // Extract the ID:xxx attribute to get user's UID
        const idAttribute = attrArray.find((attr: string) =>
          attr.startsWith("ID:"),
        );

        if (idAttribute) {
          // Remove "ID:" prefix to get the actual UID
          setUserUID(idAttribute.substring(3));
        } else {
          // Fallback: use userId from response (or placeholder)
          setUserUID(`ABE-USR-${data.userId}`);
        }

        // Filter out ID attributes, keep only business attributes for display
        const businessAttributes = attrArray.filter(
          (attr: string) => !attr.startsWith("ID:"),
        );

        // Convert each attribute string into an Attribute object with color theme
        const parsedAttributes = businessAttributes.map(
          (attrStr: string, index: number) => {
            const parts = attrStr.split(":");
            // Determine category: if colon exists, use left part, else default "tag"
  ...
  // --------------------------------------------------------------------------
  // Copy UID to clipboard with temporary success feedback
  // --------------------------------------------------------------------------
  const handleCopyUID = () => {
  ...
  return (
    <div className="rounded-xl p-4 sm:p-6 bg-white dark:bg-slate-900/40 backdrop-blur-md border border-slate-200 dark:border-slate-700/50 shadow-sm dark:shadow-none transition-colors duration-300">
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="font-display text-lg sm:text-xl text-slate-900 dark:text-white">
          Identity & Key Status
        </h2>
        {loading && (
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          {loading ? (
            ...
            // Empty state when no attributes assigned
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
            These attributes define your access permissions in policy-based
            sharing
          </p>
        </div>
        ...
        <div className="p-3 sm:p-4 rounded-lg bg-slate-50 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700/50 transition-colors">
          <div className="flex items-start gap-2 sm:gap-3">
            ...
            <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
              Your cryptographic identity is securely managed. All file access
              is verified against your attributes.
            </p>
          </div>
        </div>
      </div>
